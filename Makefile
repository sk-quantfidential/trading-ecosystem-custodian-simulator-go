# Custodian Simulator Go - Makefile
...
# Build targets
build: ## Build the custodian simulator binary
	@echo "Building custodian simulator..."
	go build -o custodian-simulator ./cmd/server

clean: ## Clean build artifacts
	@echo "Cleaning..."
	rm -f custodian-simulator server
	go clean -testcache
...
test-files: ## Show test files
	@echo "Test files in custodian-simulator-go:"
	@find . -name "*_test.go" -exec echo "  {}" \;

# Status check
status: ## Check current test status
	@echo "=== Custodian Simulator Go Test Status ==="
	@echo ""
